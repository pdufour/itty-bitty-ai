...
// Define the trait here to avoid circular dependencies
pub trait TokenCallback {
    fn on_token(&mut self, token: &str) -> anyhow::Result<()>;
    fn on_complete(&mut self, rest: Option<String>) -> anyhow::Result<()>;
}
...
impl CausalSelfAttention {
    ...
    pub(crate) fn forward(&self, x: &Tensor, index_pos: usize, block_idx: usize) -> Result<Tensor> {
...
impl Llama {
    ...
    pub fn generate(
        ...
        for _ in 0..max_tokens {
            let input = Tensor::new(&tokens[tokens.len() - 1..], &device)?;

            let logits = self.forward(&input, tokens.len() - 1)?;

            let next_token = logits.argmax(1)?.squeeze(0)?.to_scalar::<u32>()?;

            let token_text = format!(" {}", next_token);

            if let Err(e) = callback.on_token(&token_text) {
                return Err(anyhow::anyhow!("Callback error: {}", e));
            }
        ...
        if let Err(e) = callback.on_complete(None) {
            return Err(anyhow::anyhow!("Callback error: {}", e));
        }
