...
fn format_from_style(
    ...
    // Get base monospace font
    let base_font = TextStyle::Monospace.resolve(egui_style);

    // Adjust size based on style while keeping monospace family
    let font_id = if emark_style.heading {
        FontId::new(base_font.size * 1.5, base_font.family.clone())
    ...
    egui::text::TextFormat {
        font_id, // Use monospace font for everything
        color,
        background,
